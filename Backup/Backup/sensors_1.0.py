...
def calibration():
	...
	i=1
	while (i <= iterations):
		# Citește valoarea brută de la accelerometru
		...
		# Citește valoarea brută de la giroscop
	...
	# Calculează media valorilor de offset pentru accelerometru	
	Axoffset = round((Axoffset/200), 3)
	Ayoffset = round((Ayoffset/200), 3)
	Azoffset = round((Azoffset/200), 3)
    # Calculează media valorilor de offset pentru giroscop
	Gxoffset = round((Gxoffset/200), 3)
	Gyoffset = round((Gyoffset/200), 3)
	Gzoffset = round((Gzoffset/200), 3)
	
	baseline_values = []
	# Colectează valori de presiune pentru a stabili valoarea de bază
	for i in range(iterations):
		pressure = bmp280.get_pressure()
		baseline_values.append(pressure)
		time.sleep(s)
	# Calculează media valorilor de presiune, excluzând ultimele 25 de valori
	Preasure = sum(baseline_values[:-25]) / len(baseline_values[:-25])
	# Returnează valorile calculate pentru offset-uri și presiune
...
def MPU_read(Axoffset, Ayoffset, Azoffset, Gxoffset, Gyoffset, Gzoffset):
...
def Run():
	...
	MPU_Init()
	MPU_read(Axoffset, Ayoffset, Azoffset, Gxoffset, Gyoffset, Gzoffset)
	LastTime = time.time()
	Pitch = 0
	Roll  = 0
	...
	while True:
		...
		#print ("Roll =", Roll, "\tPitch =", Pitch, "\tAltitude =", Altitude, "metres")
